...
// Tu codigo acá:
var toDoItems = []; // importante escribirlo para 'destrabar' tests
...
// Tu código acá:
var span  = document.querySelector("#createdBy"); // se accede a id con # 
span.innerHTML + 'Trini' // para que no se pise span, escribo asi 

//query selector gets first element but theres only one id 
//innerHTML returns content previously selected element.innerHTML also works
...
function ToDo (description) { // a cada elemento le genera un ToDo
  // Tu código acá:
...
// Tu código acá:
ToDo.prototype.completeToDo = function () { //agrego metodo al prototipo de ToDo
  this.complete = true; // quiero hacer que el complete sea igual a true
}
...
function buildToDo(todo, index) {
  // Tu código acá:
  let toDoShell = document.createElement("div"); // genera <div class = 'toDoShell'> </div> 
  toDoShell.className = 'toDoShell';  
  let toDoText = document.createElement("span");
  toDoText.innerHTML = todo.description; // genera <span id= index> descripcion </span>
  toDoText.id = index; 
  if(toDoText.complete == true) {
    toDoText.className = 'completeText'; 
  } 
  toDoShell.appendChild(toDoText); //agrega elementos de js a HTML 
...
function addToDo() {
  ...
  if(input.value !== "") { // si input.value es diferente de un valor vacio 
    let toDo1 = new ToDo(input.value);// crea esto {description=input.value, complete=false}
